...
class SixDofAnimation(BaseAnimationModule):
    ...
    def __init__(
        self,
        expr_config_path: str = 'configs/multiagent-weighted-six-dof-inspection/experiment.yml',
        task_config_path: str = 'configs/multiagent-weighted-six-dof-inspection/task.yml',
    ...
    def get_data_from_dataframe(self, dataframe):
        ...
        for state in dataframe["ObservationVector"][0]:
            ...
            inspected_points.append(state.data['Obs_Sensor_InspectedPoints'].value['direct_observation'].value)
            sun_angles.append(state.data['Obs_Sensor_SunAngle'].value['direct_observation'].value[0])
            quaternions.append(state.data['Obs_Sensor_Quaternion'].value['direct_observation'].value)
            angular_velocities.append(state.data['Obs_Sensor_AngularVelocity'].value['direct_observation'].value)
            try:
                reaction_wheel_velocities.append(state.data['Obs_Sensor_ReactionWheelVelocity'].value['direct_observation'].value)
                temperatures.append(state.data['Obs_Sensor_Temperature'].value['direct_observation'].value)
                energy.append(state.data['Obs_Sensor_Energy'].value['direct_observation'].value)
            except KeyError:
                pass
    ...
    def setup_plots(self, data):
        ...
        points = InspectionPoints.points_on_sphere_fibonacci(self.num_points, self.radius)
        for p in points:
...
if __name__ == '__main__':
    animation = SixDofAnimation(checkpoint_path='output/tune/MULTIAGENT-SIX-DOF-INSPECTION/MULTIAGENT-SIX-DOF-INSPECTION-PPO_CorlMultiAgentEnv_006b1_00000_0_2026-04-13_08-56-50/checkpoint_000065')
